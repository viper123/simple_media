...
class AudioPlayer : AudioPlayerInstance {
    ...
    var duration : CMTime? {
        ...
        if (!duration.isValid || duration.isIndefinite) {
            guard let assetDuration = currentItem?.asset.duration else {
                return nil
            }
    ...
    private func updateDuration() {
        guard let toUpdateDuration = duration else {
...
extension AVPlayerItem {
    private struct AssociatedKeys {
        static var customTag: UInt8 = 0
        static var userInfo: UInt8 = 0
    }
