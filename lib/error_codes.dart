...
extension ErrorCodesMeaning on Error {
  String get errorMeaning => switch (code) {
    ...
    ErrorCode.androidInvalidState => "Caused by a command that cannot be completed because the current state is not valid",
    ErrorCode.androidBadValue => "Caused by an argument that is illegal",
    ErrorCode.androidPermissionDenied => "Caused by a command that is not allowed",
    ErrorCode.androidNotSupported => "Caused by a command that is not supported",
    ErrorCode.androidDisconnected => "Caused by a disconnected component",
    ErrorCode.androidAuthenticationExpired => "Caused by expired authentication",
    ErrorCode.androidPremiumAccountRequired => "Caused by a premium account that is required but the user is not subscribed",
    ErrorCode.androidConcurrentStreamLimit => "Caused by too many concurrent streams",
    ErrorCode.androidParentalControlRestricted => "Caused by the content being blocked due to parental controls",
    ErrorCode.androidNotAvailableInRegion => "Caused by the content being blocked due to being regionally unavailable",
    ErrorCode.androidSkipLimitReached => "Caused by the skip limit that is exhausted",
    ErrorCode.androidSetupRequired => "Caused by playback that needs manual user intervention",
    ErrorCode.androidEndOfPlaylist => "Caused by navigation that failed because the playlist was exhausted",
    ErrorCode.androidContentAlreadyPlaying => "Caused by a request for content that was already playing",
    ErrorCode.androidUnspecified => "Caused by an error whose cause could not be identified",
    ErrorCode.androidRemoteError => "Caused by an unidentified error in a remote Player, which is a Player that runs on a different host or process",
    ErrorCode.androidBehindLiveWindow => "Caused by the loading position falling behind the sliding window of available live content",
    ErrorCode.androidTimeout => "Caused by a generic timeout",
    ErrorCode.androidFailedRuntimeCheck => "Caused by a failed runtime check",
    ErrorCode.androidIOUnspecified => "Caused by an Input/Output error which could not be identified",
    ErrorCode.androidIONetworkConnectionFailed => "Caused by a network connection failure",
    ErrorCode.androidIONetworkConnectionTimeout => "Caused by a network timeout, meaning the server is taking too long to fulfill a request",
    ErrorCode.androidIOInvalidHTTPContentType => "Caused by a server returning a resource with an invalid 'Content-Type' HTTP header value",
    ErrorCode.androidIOBadHttpStatus => "Caused by an HTTP server returning an unexpected HTTP response status code",
    ErrorCode.androidIOFileNotFound => "Caused by a non-existent file",
    ErrorCode.androidIONoPermission => "Caused by lack of permission to perform an IO operation",
    ErrorCode.androidIOCleartextNotPermitted => "Caused by the player trying to access cleartext HTTP traffic",
    ErrorCode.androidIOReadPositionOutOfRange => "Caused by reading data out of the data bound",
    ErrorCode.androidParsingContainerMalformed => "Caused by a parsing error associated with a media container format bitstream",
    ErrorCode.androidParsingManifestMalformed => "Caused by a parsing error associated with a media manifest",
    ErrorCode.androidParsingContainerUnsupported => "Caused by attempting to extract a file with an unsupported media container format, or an unsupported media container feature",
    ErrorCode.androidParsingManifestUnsupported => "Caused by an unsupported feature in a media manifest",
    ErrorCode.androidDecoderInitFailed => "Caused by a decoder initialization failure",
    ErrorCode.androidDecoderQueryFailed => "Caused by a decoder query failure",
    ErrorCode.androidDecodingFailed => "Caused by a failure while trying to decode media samples",
    ErrorCode.androidDecodingFormatExceedsCapabilities => "Caused by trying to decode content whose format exceeds the capabilities of the device",
    ErrorCode.androidDecodingFormatUnsupported => "Caused by trying to decode content whose format is not supported",
    ErrorCode.androidDecodingResourcesReclaimed => "Caused by higher priority task reclaiming resources needed for decoding",
    ErrorCode.androidAudioTrackInitFailed => "Caused by an AudioTrack initialization failure",
    ErrorCode.androidAudioTrackWriteFailed => "Caused by an AudioTrack write operation failure",
    ErrorCode.androidAudioTrackOffloadWriteFailed => "Caused by an AudioTrack write operation failure in offload mode",
    ErrorCode.androidAudioTrackOffloadInitFailed => "Caused by an AudioTrack init operation failure in offload mode",
    ErrorCode.androidDrmUnspecified => "Caused by an unspecified error related to DRM protection",
    ErrorCode.androidDrmSchemeUnsupported => "Caused by a chosen DRM protection scheme not being supported by the device",
    ErrorCode.androidDrmProvisioningFailed => "Caused by a failure while provisioning the device",
    ErrorCode.androidDrmContentError => "Caused by attempting to play incompatible DRM-protected content",
    ErrorCode.androidDrmLicenseAcquisitionFailed => "Caused by a failure while trying to obtain a license",
    ErrorCode.androidDrmDisallowedOperation => "Caused by an operation being disallowed by a license policy",
    ErrorCode.androidDrmSystemError => "Caused by an error in the DRM system",
    ErrorCode.androidDrmDeviceRevoked => "Caused by the device having revoked DRM privileges",
    ErrorCode.androidDrmLicenseExpired => "Caused by an expired DRM license being loaded into an open DRM session",
    ErrorCode.androidVideoFrameProcessorInitFailed => "Caused by a failure when initializing a VideoFrameProcessor",
    ErrorCode.androidVideoFrameProcessingFailed => "Caused by a failure when processing a video frame",
  };
}
